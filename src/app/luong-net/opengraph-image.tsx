...
export const alt = "Tính lương Net — 1phantram.com";
...
// TNCN luỹ tiến 7 bậc (tháng, VND)
function tncn(taxable: number): number {
...
export default async function Image(_props: { searchParams?: SP } = {}) {
  ...
  const color = { light: "#d1fae5", mid: "#10b981", dark: "#047857" };
  const hasData = g > 0;

  let numberDisplay = "?";
  let statusLabel = "Tính lương Net online";
  let stats: Array<{ value: string; label: string }> = [];

  if (hasData) {
    const bhCap = Math.min(g, 46_800_000); // 20× lương cơ sở 2024 (~2.34M × 20)
    const bh = bhCap * 0.105;
    const personalDeduction = 11_000_000 + Math.max(0, d) * 4_400_000;
    const taxable = Math.max(0, g - bh - personalDeduction);
    const tax = tncn(taxable);
    const net = g - bh - tax;
    const pct = Math.round((net / g) * 100);
    numberDisplay = formatVND(net);
    statusLabel = `Nhận về ${pct}% gross`;
    stats = [
      { value: formatVND(g), label: "Lương Gross" },
      { value: formatVND(bh + tax), label: "Thuế + BH" },
      { value: `${Math.max(0, d)} người`, label: "Phụ thuộc" },
    ];
  }
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: color.dark, color: "white", padding: "12px 20px", borderRadius: 999, fontSize: 16, fontWeight: 700, letterSpacing: 1, boxShadow: "0 8px 20px rgba(0,0,0,0.15)" }}>
            ...
            <span style={{ display: "flex" }}>LƯƠNG NET</span>
          </div>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "center", marginTop: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, background: "rgba(15,23,42,0.85)", color: "white", padding: "14px 32px", borderRadius: 999, fontSize: 22, fontWeight: 700, boxShadow: "0 10px 30px rgba(0,0,0,0.25)" }}>
            ...
            <span style={{ display: "flex" }}>Tính ngay tại 1phantram.com/luong-net</span>
          </div>
        </div>
      </div>
    ),
